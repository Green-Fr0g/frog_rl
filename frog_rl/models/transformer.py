"""Transformer encoder with self-attention and optional cross-attention."""
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        **_: object,
        ...
        if ff_hidden_dim is not None and ff_hidden_dim <= 0:
    ...
    def as_jit(self) -> nn.Module:
        """Return a self-attention wrapper suitable for TorchScript export."""
        return _TorchTransformer(self)

    def as_onnx(self, verbose: bool = False) -> nn.Module:
        """Return a self-attention wrapper suitable for ONNX export."""
        return _OnnxTransformer(self, verbose)

    def as_cross_attention_jit(self) -> nn.Module:
        """Return a cross-attention wrapper suitable for TorchScript export."""
        return _ExportableCrossAttentionTransformer(self)

    def as_cross_attention_onnx(self, verbose: bool = False) -> nn.Module:
        """Return a cross-attention wrapper suitable for ONNX export."""
        return _ExportableCrossAttentionTransformer(self, verbose=verbose)
...
class _TorchTransformer(nn.Module):
    """TorchScript export wrapper for self-attention inference."""
...
class _OnnxTransformer(nn.Module):
    """ONNX export wrapper for self-attention inference."""

    def __init__(self, model: Transformer, verbose: bool) -> None:
        super().__init__()
        self.model = copy.deepcopy(model)
...
class _ExportableCrossAttentionTransformer(nn.Module):
    """Export wrapper for cross-attention with explicit query, key, and value inputs."""

    def __init__(self, model: Transformer, verbose: bool = False) -> None:
        super().__init__()
        self.model = copy.deepcopy(model)
        self.q_dim = model.q_dim
        self.k_dim = model.k_dim
        self.v_dim = model.v_dim
        self.max_seq_len = model.max_seq_len
        self.verbose = verbose

    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
        return self.model(q=query, k=key, v=value)

    def get_dummy_inputs(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        return (
            torch.zeros(1, self.max_seq_len, self.q_dim),
            torch.zeros(1, self.max_seq_len, self.k_dim),
            torch.zeros(1, self.max_seq_len, self.v_dim),
        )
